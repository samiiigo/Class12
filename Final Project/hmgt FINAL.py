...
def mainapp():
    ...
    def logout():
        ...
        CTkLabel(logout_,text = 'Logging Out...',font = ('Lexend',20,'bold')).place(relx = 0.5,rely = 0.6,anchor = CENTER)
        progressbar.start()
        
        def logged_out():
            progressbar.stop()
            CTkLabel(logout_,height = 200,text = 'Logged Out Successfully!',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)
            tabview.set("About")
            update_logbutton()
    ...
    def loggin():
        ...
        def save_signin():
            ...
            if misc._text == '':
                ...
                progressbar.place(relx = 0.5,rely = 0.5, anchor = CENTER)
                CTkLabel(signin_,text = 'Signing In...',font = ('Lexend',20,'bold')).place(relx = 0.5,rely = 0.6,anchor = CENTER)
                progressbar.start()
                ...
                def logged_in(progress):
                    ...
                    cur0.close()
                    sqlcon.commit()
                    CTkLabel(signin_,height = 620,width = 540,corner_radius=20,text = 'Signed In Successfully!',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)
                    update_logbutton()
        ...
        login_ = CTkLabel(signup,text = 'Log In', text_color = '#25e7ff')
        ...
        def save_login():
            ...
            if passw == '':
                ...
                if passw == password[0][0][0]:
                    ...
                    progressbar.place(relx = 0.5,rely = 0.5, anchor = CENTER)
                    CTkLabel(logoin_,text = 'Logging In...',font = ('Lexend',20,'bold')).place(relx = 0.5,rely = 0.6,anchor = CENTER)
                    progressbar.start()

                    def logged_in(progress):
                        progress.stop()
                        cur0.close()
                        CTkLabel(logoin_,height = 200,corner_radius=20,text = 'Logged In Successfully!',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)
                        update_logbutton()
                        root.after(2000,lambda:frame_login.destroy())
        ...
        signup_ = CTkLabel(login,text = 'Sign Up', text_color = '#25e7ff')
    ...
    def update_logbutton():
        def onEnter(event): button_login.configure(text=' LOGOUT ',text_color='#FFFFFF',image=profile_pic_)

        def onLeave(event): button_login.configure(text='',image=profile_pic)

        if loggedin[0] is True:
            profile_pic = CTkImage(Image.open('assets/logout.png'),size = (50,50))
            profile_pic_ = CTkImage(Image.open('assets/logout_.png'),size = (50,50))
            button_login.configure(text = '',image=profile_pic)
            button_login.bind("<Button-1>", lambda x:logout())
            button_login.bind('<Enter>',  onEnter);button_login.bind('<Leave>',  onLeave)
        elif loggedin[0] is False:
            button_login.configure(text = '',image=profile_pic0)
            button_login.bind("<Button-1>", lambda x:loggin())
            button_login.bind('<Enter>',  onenter);button_login.bind('<Leave>',  onleave)
    ...
    profile_pic0 = CTkImage(Image.open('assets/login.png'),size = (50,50))
    profile_pic0_ = CTkImage(Image.open('assets/login_.png'),size = (50,50))
    button_login = CTkLabel(frame,text = '',fg_color='#4a5a95',image=profile_pic0)
    button_login.place(relx = 0.97,rely = 0.058,anchor = CENTER)
    button_login.bind("<Button-1>", lambda x:loggin())

    def onenter(event):button_login.configure(text='LOGIN',text_color='#242b48',image=profile_pic0_)

    def onleave(event):button_login.configure(text='',image=profile_pic0)
    ...
    pic = CTkScrollableFrame(about,height = 1136,width = 1920,corner_radius=15)
    pic.place(relheight = 1, relwidth = 1, x = 0,y = 0)

    image_files = ['assets/img2.jpg', 'assets/img3.jpg','assets/img4.jpg','assets/img5.jpg','assets/img6.jpg']
    images = [CTkImage(Image.open(file),size=(1920,1136)) for file in image_files]
    
    canvas = CTkLabel(pic, text = '', width = 1920, height = 1136,image=images[0])
    canvas.pack()
    ...
    mgr_pic = CTkImage(Image.open('assets/mgr.jpg'),size = (274,464))
    ...
    rec_pic = CTkImage(Image.open('assets/rec.jpg'),size = (274,464))
    ...
    serv_pic = CTkImage(Image.open('assets/serv.jpg'),size = (274,464))
    ...
    cook_pic = CTkImage(Image.open('assets/cook.jpg'),size = (274,464))
    ...
    single_pic = CTkImage(Image.open('assets/single.jpg'),size = (237,464))
    ...
    double_pic = CTkImage(Image.open('assets/double.jpg'),size = (237,464))
    ...
    triple_pic = CTkImage(Image.open('assets/triple.jpg'),size = (237,464))
    ...
    quad_pic = CTkImage(Image.open('assets/quad.jpg'),size = (237,464))
    ...
    for r in rooms_unavai:
        if r == 'Single': button1.configure(text = 'Booked', state = DISABLED)
        if r == 'Double':button2.configure(text = 'Booked', state = DISABLED)
        if r == 'Triple':button3.configure(text = 'Booked', state = DISABLED)
        if r == 'Quad':button4.configure(text = 'Booked', state = DISABLED)
    ...
    def select_datein():
        ...
        tkc1 = Calendar(fr_indate,selectmode = "day")
    ...
    CTkLabel(select_date_,text = "Checkin Date*",font = ('Lexend',17)).place(relx = 0.5,rely = 0.4, anchor = CENTER)
    ...
    def select_dateout():
        ...
        tkc1 = Calendar(fr_outdate,selectmode = "day")
        tkc1.pack(padx = 10,pady = 10)
    ...
    CTkLabel(select_date_,text = "Checkout Date*",font = ('Lexend',17)).place(relx = 0.5,rely = 0.7, anchor = CENTER)
    ...
    CTkButton(booking_,height = 100,width = 100,text = 'Next →', font = ('Lexend',17), command=lambda:next_sel() if checkin_date.get() != '' and checkout_date.get() != '' else err_msg0.configure(text = '*Please enter Date*')).place(relx = 0.85,rely = 0.5,anchor = CENTER)

    def select_type():
        select_type_ = CTkFrame(booking_,height = 540,width = 540,corner_radius=20)
        select_type_.place(relx = 0.5,rely = 0.5, anchor = CENTER)

        CTkLabel(select_type_,corner_radius=20,text = 'Please choose the type of room',font = ('Lexend',25,'bold')).place(relx = 0.5,y = 45,anchor = CENTER)

        radio_var = StringVar()

        radiobutton_1 = CTkRadioButton(select_type_, text = "Single", variable= radio_var, value='Single')
        radiobutton_2 = CTkRadioButton(select_type_, text = "Double", variable= radio_var, value='Double')
        radiobutton_3 = CTkRadioButton(select_type_, text = "Triple", variable= radio_var, value='Triple')
        radiobutton_4 = CTkRadioButton(select_type_, text = "Quad", variable= radio_var, value='Quad')
        ...
        def select_misc():
            ...
            CTkLabel(select_misc_,corner_radius=20,text = 'Enhance your stay. Select add-ons and extras:',font = ('Lexend',20,'bold')).place(relx = 0.5,rely = 0.05,anchor = CENTER)
            ...
            CTkLabel(fr_transfer,text = 'Airport Transfer', font = ('Lexend',30)).place(x = 290,rely = 0.4,anchor = CENTER)
            CTkLabel(fr_transfer,text = '$50 per unit', font = ('Lexend',15)).place(x = 210,rely = 0.6,anchor = CENTER)
            ...
            CTkLabel(fr_tour,text = 'City Tour', font = ('Lexend',30)).place(x = 235,rely = 0.4,anchor = CENTER)
            CTkLabel(fr_tour,text = '$170 per night', font = ('Lexend',15)).place(x = 220,rely = 0.6,anchor = CENTER)
            tour_var = StringVar()
            ...
            CTkLabel(fr_feast,text = 'Breakfast and Lunch', font = ('Lexend',30)).place(x = 320,rely = 0.4,anchor = CENTER)
            CTkLabel(fr_feast,text = '$100 per night', font = ('Lexend',15)).place(x = 220,rely = 0.6,anchor = CENTER)
            feast_var = StringVar()
            ...
            misc_back = CTkButton(booking_,height = 100,width = 100,text = '← Back', font = ('Lexend',17), command=lambda:back_sel())
            ...
            def show_details():
                ...
                CTkLabel(show_details_,corner_radius=20,text = 'Booking Summary:',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.1,anchor = CENTER)
                CTkLabel(show_details_,text = f"Checkin Date: {checkin_date.get()}",font = ('Lexend',17)).place(relx = 0.3,rely = 0.2, anchor = CENTER)
                CTkLabel(show_details_,text = f"Checkout Date: {checkout_date.get()}",font = ('Lexend',17)).place(relx = 0.3,rely = 0.3, anchor = CENTER)
                CTkLabel(show_details_,text = f"Number of Days: {diff}",font = ('Lexend',17)).place(relx = 0.3,rely = 0.4, anchor = CENTER)
                CTkLabel(show_details_,text = f"Type: {radio_var.get()}",font = ('Lexend',17)).place(relx = 0.75,rely = 0.2, anchor = CENTER)
                cur = sqlcon.cursor()
                cur.execute(f'select * from rooms where type = "{radio_var.get()}" and availability = "yes"')
                current_room = cur.fetchall()
                cur.close()  
                CTkLabel(show_details_,text = f"Room No: {current_room[0][0]}",font = ('Lexend',17)).place(relx = 0.75,rely = 0.3, anchor = CENTER)
                CTkLabel(show_details_,text = f"Price: ${current_room[0][3]}",font = ('Lexend',17)).place(relx = 0.75,rely = 0.4, anchor = CENTER)
                ...
                add1 = CTkLabel(addons_,text = '',font = ('Lexend',17))
                add1.place(relx = 0.5,rely = 0.5,anchor = CENTER)
                add2 = CTkLabel(addons_,text = '',font = ('Lexend',17))
                ...
                details_back = CTkButton(booking_,height = 100,width = 100,text = '← Back', font = ('Lexend',17), command=lambda:back_misc())
                ...
                def book():
                    ...
                    def print_recipt():
                        ...
                        CTkLabel(rec_,text = 'Thank You!',font = ('Lexend',20),width = 500,height = 200,corner_radius=30).place(relx = 0.5,rely = 0.5,anchor = CENTER)
                    ...
                    def recload_stop():
                        ...
                        try:
                            ...
                            CTkLabel(rec_,height = 200,text = 'Room Booked Successfully!',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)
                            ...
                            CTkLabel(rec_,height = 200,text = 'Booking Failed', width = 520,font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)
                    ...
                    progressbar.place(relx = 0.5,rely = 0.5, anchor = CENTER)
                    CTkLabel(rec_,text = 'Booking...',font = ('Lexend',20,'bold')).place(relx = 0.5,rely = 0.6,anchor = CENTER)
                    progressbar.start()
                ...
                def showsignin():
                    ...
                    CTkLabel(signupreq,corner_radius=20,text = 'Please Log In',font = ('Lexend',25,'bold')).place(relx = 0.5,rely = 0.5,anchor = CENTER)

                    root.after(2000,lambda:req.destroy())

                proceed = CTkButton(booking_,height = 100,width = 100,text = 'Book', font = ('Lexend',17), command=lambda:book() if loggedin[0] is True else showsignin())
                proceed.place(relx = 0.85,rely = 0.5,anchor = CENTER)

            misc_next = CTkButton(booking_,height = 100,width = 100,text = 'Done →', font = ('Lexend',17), command=lambda:show_details())
